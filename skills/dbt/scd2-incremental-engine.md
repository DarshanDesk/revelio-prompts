<skill_definition name="scd2_incremental_engine">
  <metadata>
    ...
    <capability>SCD Type 2 Incremental State Machine — Open-Record Logic and Surrogate Key Management</capability>
    ...
    <sourced_from>Extracted from dbt-logic-pro.agent.md inline blocks: manage_scd2_incremental + native_hashing_macros</sourced_from>
  </metadata>

  <description>
    This skill implements the SCD Type 2 incremental state machine for the
    `financial_observations_fact` EAV table in DuckDB/dbt. It manages:
    - Surrogate key derivation via native MD5 (no dbt-utils)
    - ATTR_HASH change detection
    - Open-record identification and expiry within `is_incremental()` blocks
    - New current-record insertion with correct EFF_FROM / IS_CURRENT = TRUE

    COLUMN CONTRACT (from SCHEMA.md § SCD2 Column Name Authority):
      EFF_FROM    — TIMESTAMP — inclusive start of validity
      EFF_TO      — TIMESTAMP — exclusive end; NULL = open record
      IS_CURRENT  — BOOLEAN  — physical flag; TRUE = authoritative current record
  </description>
