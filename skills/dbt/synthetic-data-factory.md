<skill_definition name="synthetic_data_factory">
  <metadata>
    ...
    <capability>CSV Seed Generation — Long-Format S&P Financial Data, EAV Attributes, and Delta Update Mocking</capability>
    <target_engine>DuckDB seeds (CSV) | dbt /seeds folder</target_engine>
    <owning_agent>risk-data-synthesizer</owning_agent>
    <second_brain_ref>.github/context-cache/RULES.md § Data Synthesis Parameters | SCHEMA.md § EAV Column Contract</second_brain_ref>
    <sourced_from>Extracted from risk-data-synthesizer.agent.md inline blocks: generate_long_format_seeds + generate_eav_attributes + mock_realtime_updates (3 blocks consolidated)</sourced_from>
  </metadata>

  <description>
    This skill generates structurally accurate, logically consistent CSV seed files for the
    dbt /seeds folder. It covers three consolidated concerns:

    1. **Long-Format Seed Generation**: Linked CSV rows across all 4 S&P files
       (Vendor, Period, Data, Ratio) and the Analyst Override and Obligor Mapping files,
       sharing a common (COMP_ID, PERIOD_ID, ATTR_ID) grain.

    2. **EAV Attribute Generation**: ATTR_VALUE as VARCHAR across multiple DATA_TYPEs
       (DECIMAL, INTEGER, DATE, VARCHAR) matching the attr_metadata_dim contract.

    3. **Delta Update Mocking (Day 0 / Day N)**: Simulates the Day 0 initial full load
       and the Day N incremental delta to trigger SCD2 open/close logic.

    SEED PARAMETERS (from RULES.md § Data Synthesis Parameters — do not redefine here):
      - Minimum 500 records
      - ≥ 5% PENDING_MAP records
      - ≥ 2 Analyst Override records
      - V1 (Day 0) + V2 (Day N) versioning
  </description>
