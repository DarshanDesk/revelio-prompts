<skill_definition name="audit_test_generator">
  <metadata>
    ...
    <capability>dbt Test Generation — Schema Tests, Custom Data Quality Tests, and Idempotency Verification</capability>
    ...
    <sourced_from>Extracted from financial-audit-pro.agent.md inline blocks: generate_custom_data_tests + idempotency_check</sourced_from>
  </metadata>

  <description>
    This skill generates native dbt test SQL (no dbt-utils) covering:

    1. **Custom Data Tests**: Range checks for Financial Ratios, null checks for mandatory
       Risk Attributes, and ATTR_VALUE defensive regex casting validation.

    2. **Idempotency Tests**: Verify that a Full Feed re-run with identical source data
       produces zero new rows — the ATTR_HASH equality contract from scd2-incremental-engine.md.

    COLUMN CONTRACT (loaded from SCHEMA.md § SCD2 Column Name Authority):
      IS_CURRENT  — BOOLEAN — filter: IS_CURRENT = TRUE. FORBIDDEN: IS_CURRENT = 1
      EFF_FROM    — TIMESTAMP — inclusive start
      EFF_TO      — TIMESTAMP — exclusive end; NULL = open record
      ATTR_VALUE  — VARCHAR(4000) — always validate as string before numeric use
  </description>
