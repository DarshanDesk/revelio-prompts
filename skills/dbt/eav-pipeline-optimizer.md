<skill_definition name="eav_pipeline_optimizer">
  <metadata>
    ...
    <capability>EAV Pipeline Processing — OBLIGOR Mapping Reconciliation and DuckDB Join Optimization</capability>
    ...
    <sourced_from>Extracted from dbt-logic-pro.agent.md inline blocks: mapping_reconciliation + performance_tuning</sourced_from>
  </metadata>

  <description>
    This skill handles the EAV-specific pipeline concerns in `financial_observations_fact`:

    1. **OBLIGOR Mapping Reconciliation**: LEFT JOIN from S&amp;P source to Internal App DB
       with COALESCE fallback to 'PENDING_MAP' sentinel — enforcing Zero Data Loss.

    2. **DuckDB EAV Join Optimization**: Patterns for efficient large-scale EAV joins
       with portability annotations for Oracle/Hive targets.

    COLUMN CONTRACT (from SCHEMA.md § EAV Column Contract):
      ATTR_VALUE  — VARCHAR(4000) — single unified storage column. NEVER split into typed columns.
      OBLIGOR_ID  — nullable in company_dim; PENDING_MAP applied at model layer via COALESCE.
  </description>
